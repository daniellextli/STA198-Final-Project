...
```{r}
m_3 <- glm(high_caffeine ~ Age + Gender + Country+ BMI + Heart_Rate + Sleep_Quality,
           data = data,
           family = "binomial")
```
\begin{table}[H]
...
Sleep Quality: Fair & 0.980 & $1.05 \times 10^{-14}$ \\
Sleep Quality: Good & 0.614 & $2.35 \times 10^{-7}$ \\
Sleep Quality: Poor & 1.34 & $< 2 \times 10^{-16}$ \\
\hline
\end{tabular}
\caption{Logistic Regression of High Caffeine Intake (With Demographic Predictors)}
\end{table}
